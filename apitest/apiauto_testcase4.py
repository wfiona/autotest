...
class ApiFlow(unittest.TestCase):
    ...
    def test_readSQLcase(self):
        ...
        coon = pymysql.connect(user='root',passwd='santang123',db='wll_test',port=3306,host='192.168.1.219',charset='utf8')
        cursor = coon.cursor()
    ...
    for case in case_list:
        ...
        if method.upper() == 'GET':
            ...
            if "=" in urlParam(param):
                data = None
                print (str(case_id)+' request is get' +new_url.encode('utf8')+'?'+urlParam(param).encode('utf-8'))
        ...
        if method.upper()=="POST":
            ...
            if "=" in urlParam(param):
                ...
                results = requests.post(new_url,data=urlParam(param).encode('utf8'),headers=headers).text
                print (' response is post'+results.encode('utf-8'))
                responses.append(results)
                res = readRes(results,res_check)
...
if __name__ == '__main__':
    ...
    filename="E:\\autotest\\apitest\\templates\\"+"apitest_report.html"
    fp=open(filename,'wb')
